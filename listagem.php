<?php
	...
	if($pesquisar != "" && isset($_SESSION['normal'])){
		$resultPergunta     = "SELECT * FROM pergunta WHERE pergunta LIKE '%$pesquisar%' OR titulo LIKE '%$pesquisar%' LIMIT 5";
		...
		<html lang="pt-br">
			...
			<body>
								...
								<ul class="navbar-nav me-auto mb-2 mb-lg-0">
									...
									<li class="nav-item">
										<a class="nav-link active" href="index.php" tabindex="-1" aria-disabled="true">
											<span class="itensNav" id="a4" style="color: black;">Faça uma pergunta</span>
										</a>
									</li>
								</ul>
							</div>
				...
				<div class="container theme-showcase" role="main">
					...
					<?php
						if(($resultadoPergunta) AND ($resultadoPergunta->num_rows != 0 )){
							while($rowsPergunta = mysqli_fetch_assoc($resultadoPergunta)){
					?>
								<hr></hr>
								<div class="thumbnail">
									<div class="caption">
										<a href="detalhes.php?ID_pergunta=<?php echo $rowsPergunta['IDpergunta']; ?>">
											<h5><?php echo $rowsPergunta['titulo']; ?></h5>
										</a>
										<p><?php echo $rowsPergunta['pergunta']; ?></p>
									</div>
								</div>
								</hr>
					<?php 
							...
							echo "<br><div style='text-align: center; font-size: 17px'> Não conseguimos encontrar nada para '<b>$pesquisar</b>' </div> <div style='text-align: center; font-size: 13px'> Experimente palavras-chave diferentes ou menos específicas. </div>";
						}
					?>
				</div>
...
<?php
	}else if($pesquisar == "" && isset($_SESSION['normal'])){
		$resultPergunta = "SELECT * FROM pergunta ORDER BY IDpergunta DESC";
		$resultadoPergunta = mysqli_query($conexao, $resultPergunta);
?>
		<!DOCTYPE html>
		<html lang="pt-br">
			...
			<body>
				...
				<h3 style="text-align: center;">Perguntas cadastradas</h3>
				<div class="container theme-showcase" role="main">
					<?php
						while($rowsPergunta = mysqli_fetch_assoc($resultadoPergunta)){
					?>
						...
						<div class="collapse navbar-collapse" id="navbarTogglerDemo01">
							...
							<div class="mx-auto order-0">
								<ul class="navbar-nav me-auto mb-2 mb-lg-0">
									...
									<li class="nav-item">
										<a class="nav-link active" href="index.php" tabindex="-1" aria-disabled="true">
											<span class="itensNav" id="a4" style="color: black;">Voltar para página anterior</span>
										</a>
									</li>
								</ul>
							</div>
